...
function show(selector) {
  ...
  requestAnimationFrame(() => {
    ...
    // trigger pop-seq animation inside popup
    triggerPopSeq(el);
  });
}
...
document.querySelectorAll(".point-wrapper").forEach(wrapper => {
  const trigger = wrapper.querySelector(".point-trigger");

  // Get all points
  const points = [...wrapper.querySelectorAll(".point")];

  // Get unique sorted order values
  const orders = [...new Set(points.map(p => Number(p.dataset.order)))]
    .sort((a, b) => a - b);
  ...
  trigger.addEventListener("click", () => {
    ...
    // Show all points with this order
    points
      .filter(p => Number(p.dataset.order) === currentOrder)
      .forEach(point => {
        point.offsetHeight; // force reflow
...
// Pop-seq: set --i for each child element
function triggerPopSeq(el) {
  if (!el) return;
  const children = [...el.querySelectorAll('.pop-seq > *')];
  children.forEach((child, i) => {
    child.style.setProperty('--i', i);
    child.style.animation = 'none'; // reset previous animation
    child.offsetHeight;             // force reflow
    child.style.animation = '';     // let CSS animation run again
  });
}
